...
import sys
sys.stdout = open('shapePlotter.C','w')
...
for isig in range(0,len(data.list_mc_sig)+1):
    for ibkg in range(0,len(data.list_mc_bkg)+1):
       print 'void shapePlotter::plotS%iB%iD0('%(isig,ibkg),
       if isig>0 or ibkg>0:
           print 'string VarName,TPaveText *txtGen ,',
       for ihs in range(0,isig):
           print 'TH1F *hs%i'%(ihs+1),
       ...
       print 'pCMS->AddText("CMS Phase 2 #it{Preliminary}                    ");' 
       ...
       for ihb in range(0,ibkg):
           ...
           print 'hb%i->SetLineColor(%i);'%(ihb+1,ihb+1) 
           ...
           print 'leg->AddEntry(hb%i,h%sL_bkg,"l");'%(ihb+1,ihb+1)

       for ihs in range(0,isig):
           print 'hs%i->SetFillStyle(1);'%(ihs+1)
           print 'hs%i->SetLineColor(%i);'%(ihs+1,10+ihs+1)
       ...
       print 'c->cd();'
       for ihb in range(0,ibkg):
           print 'hb%i->Scale(1/hb%i->Integral());'%(ihb+1,ihb+1)
           print 'hb%i->Draw("same");'%(ihb+1)
       #print 'hallbkg->Draw("hist");'
       #print 'hallbkg->GetYaxis()->SetTitle("Events");'
       #print 'hallbkg->Draw("hist");'
       for ihs in range(0,isig):
          print 'hs%i->Scale(1/hs%i->Integral());'%(ihs+1,ihs+1)
      ...
      # print 'h1->Draw("P,E1 SAME");'
       ...
       print 'void shapePlotter::plotS%iB%iD1('%(isig,ibkg),
       print 'string VarName,TPaveText *txtGen ,',
       for ihs in range(0,isig):
           print 'TH1F *hs%i, '%(ihs+1),
           print 'string sig_proc%s, '%(ihs+1)
